...
function initGame() {
    ...
    logo.onload = function () {
        context.drawImage(logo, 0, 0, 450, 450);  // 背景填充整个 canvas
        initChessBoard();
    }
    ...
    function initChessBoard() {
        for (let i = 0; i < 15; i++) {
            ...
            context.lineTo(15 + i * 30, 435);  // 适当调整，确保棋盘格子能完整显示
            context.stroke();
            context.moveTo(15, 15 + i * 30);
            context.lineTo(435, 15 + i * 30);  // 同上，确保每一条线都能显示
            context.stroke();
        }
    }
    ...
    function send(row, col) {
        ...
        websocket.send(JSON.stringify(req));//js转json
    }
